  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [dot, evt] = await Promise.all([
        ...
        portalSupabase.from('event_participants').select('id, event_id, events(name, event_date)').eq('employee_id', eid!).eq('signed', false),
      ]);
      return {
        dotacion: dot.data || [],
        eventos: evt.data || [],
        ...
        <div className="space-y-6">
          ...
          <Section title="Eventos / Capacitaciones">
            {data?.eventos.length === 0 ? <Empty /> : data?.eventos.map((e: any) => (
              <Item key={e.id} title={e.events?.name || 'Evento'} subtitle={`Fecha: ${e.events?.event_date ?? 's/f'}`} />
            ))}
          </Section>
        </div>
      )}
